...
- [x] **4.4 发布 v0.1.0**（已完成 2026-09-12）
  - [x] 版本号、tag、发布包（正斜杠 zip 教训沿用：.NET ZipArchive 打包并验证 0 反斜杠条目）
  - [x] GitHub 仓库初始化与 tag `v0.1.0`（远端 `lymanzhang/Bit2AtomPlotGRBLWebUI`，Release 附源码 zip，RELEASE_NOTES 落档）
...
| M5 | 阶段五 | CI 常绿、真机验收清单全通过、`$` 参数基线落档，发布 `v0.2.0` |

---

## 9. 阶段五：v0.2.0 真机验收与数据校准（规划于 2026-09-12）

**目标**：从「模拟器验证」走向「真机验证」——v0.1.0 已完成功能全量移植与模拟器/集成测试验证，v0.2.0 补齐所有依赖真机数据的欠账，并以数据校准主机侧模型。

### 任务清单

**P0 先决事项**（后续各项的前提）

- [ ] **5.1 真机全功能验收**（承接 4.2，唯一未完成的阶段四任务）
  - [ ] 按清单逐项验收并留档（附任务日志）：绘制 / 暂停 / 回溯 / 补画 / 归位 / 取消 / 超界拒绝
  - [ ] 验收发现的问题集中修复（预期为主要工作量来源，按问题性质拆分修复批次）
  - [ ] 断连保护与 `$H` 归位失败路径（penPosKnown 未知 → `/home` 回填）实测演练
- [ ] **5.2 CI 激活与加固**（不依赖真机，可立即启动）
  - [ ] 修复 `lint.yml` 触发分支：`main` → `master`（当前 CI 从未运行过）
  - [ ] workflow 补 `npm run lint` 与 `npm run typecheck` 步骤（当前仅 `npm test`）
  - [ ] 评估 Windows matrix 上 `@serialport/bindings-cpp` 原生模块构建（现行 exclude 20.x/22.x/26.x 的合理性）
- [ ] **5.3 长时任务实测**
  - [ ] 数小时级绘制一次，验证任务日志完整性、预计/实际距离同口径（仅统计笔落段）、暂停/恢复后位置双源校验表现

**P1 真机数据驱动的改进**（依赖 5.1/5.3 产出数据）

- [ ] **5.4 `$` 参数推荐基线文档**（CHANGELOG [Unreleased] 已预告）
  - [ ] 实测值写入 `docs/DEVICE_NOTES.md`：Z 轴速度/加速度、`$10` 状态回报配置、XY 限速/限加速度实测基线
  - [ ] 各固件（GRBL 1.1 / grblHAL / 厂商分支）实测差异与预设档建议同步更新
- [ ] **5.5 预计时长估算校准**（风险 7.4「双规划器语义偏差」的闭环）
  - [ ] 采集主机估算 vs 设备实际执行时长偏差数据，校准受 `$110–$122` 钳制的估算模型
- [ ] **5.6 写入 `$100–$102` 后的校准闭环**（风险 7.4 应对落地确认）
  - [ ] 核对「一键写入后提示试绘校准」现状；缺则补 10mm 校准方格一键试绘工具 + 绘制比例验证指引

**P2 可选增强**（视进度取舍）

- [ ] **5.7 GitHub Pages（WebSerial 版 UI）部署验证**：`publish.yml` 已有 workflow，真机验证浏览器直连全链路后启用
- [ ] **5.8 grblHAL / Classic 预设档扩充**：真机实测差异回填后扩展内置预设档

### 验收标准

1. CI 常绿（lint 零告警 + typecheck + 全量测试），master 分支每次推送自动执行
2. 真机全功能验收清单逐项通过并留档（附任务日志）
3. `$` 参数推荐基线写入 DEVICE_NOTES.md，预计时长估算偏差有实测数据支撑
4. 长时任务日志完整、统计口径正确（预计/实际距离同口径）
5. 版本号、tag、发布包三者一致（沿用 v0.1.0 发布流程）
