...
> 📖 **新读者请从 [项目介绍](PROJECT_INTRODUCTION.md) 开始** —— 完整覆盖项目定位、核心功能、系统架构、可靠性设计与快速上手
...
| **质量保障** | 20 个测试用例 | ✅ 53 个测试用例（回溯/补画归位/超界校验/裁剪过滤/尺寸推算专项测试）+ lint 零告警 |

### 代码规模变化

| 模块 | SAXI | 本项目 | 增量 |
| --- | ---: | ---: | --- |
| ui.tsx（界面） | 1304 行 | 2606 行 | +100%（回溯控制/着色/网格标尺/硬件配置/导出/排版与缩放设置） |
| planning.ts（运动规划） | 675 行 | 883 行 | +208 行（回溯核心算法 + 自定义传动参数 + 三态缩放选项） |
| hiding.ts（隐藏线去除） | — | 181 行 | **新增** |
| export-svg.ts（SVG 导出） | — | 68 行 | **新增** |
| plot-log.ts（任务日志） | — | 120+ 行 | **新增** |
| 测试（9 个文件 53 用例） | 20 用例 | 53 用例 | **+165%** |
...
- **纸张与排版设置** — 内置数十种预设纸张（A/B/Arch/US 系列等）+ 自定义宽高（mm）、横版/竖版、边距可调；排版锚点九宫格（水平左中右 × 垂直上中下）+ 自定义 X/Y 偏移，精确控制图形落纸位置
- **缩放模式三态** — 「等比缩放到纸张」（默认）/「按原尺寸绘制 (1:1)」/「自定义缩放比例」三选一（排版设置分区）；非 fit 模式可配合「裁剪至边距」移除超出纸张绘图区域的部分，区域内节点与线段完整保留
- **SVG 真实尺寸自动检测** — 导入时若根元素 `width` 带绝对物理单位（mm/cm/in/pt/pc/q），或 px 数值与 viewBox 不一致（如 2x/300dpi 导出），自动按 `width 实际毫米 ÷ viewBox 宽` 推算真实尺度，非 96dpi 导出的文件也能按原尺寸/自定义比例正确绘制（`width="100%"` 时物理尺寸无法恢复，按 96dpi 处理）
- **工作范围校验（防撞轴）** — 绘制前服务端校验计划坐标不超出机械行程（v3/brushless 430×300 mm，nextdraw-2234 559×864 mm），超界直接拒绝并提示超界方向与数值；预览中超界区域标红警示。custom 硬件可在硬件设置中配置「工作区宽/高 (mm)」参与校验
...
- **全命令超时保护** — 所有 EBB 命令均带超时（常规 15s、长行程按估计时长动态计算 + 60s 裕量），超时后清空队列、抬笔兜底、断使能并弹窗提示，不会卡死界面
- **LM 速率钳制** — 运动命令速率自动钳制 ≤24999 步/s，防止固件 32 位相位累加器溢出导致运动引擎停摆
- **串口写入错误自恢复** — USB 瞬断（错误码 31）等写入失败自动注入命令队列中止绘制，配合 unhandled rejection 兜底，服务不再静默崩溃
- **绘制任务日志** — 每次绘制/补画自动生成与源文件同名的日志文件，完整记录任务头（文件、模式、硬件、FIFO 深度、预计时长/距离/速度）、进度心跳、暂停/恢复/回溯事件与任务尾（实际时长/距离、结束状态），便于项目管理与故障诊断
...
**完成后颜色不自动复位**：绘制完成后界面保持「已完成任务」的状态展示（包括红色重绘标记），不会自动转入新的等待绘制状态。只有当用户点击「开始绘制」开始新任务、或取消绘制、或清除 SVG 时，红色标记才复位。这样用户有充足时间对照预览检查重复绘制区域的实际效果。
...
2. 配置传动参数：
   ...
   - 工作区宽/高 (mm)：可选，自原点 0,0 起的安全行程。配置后参与绘制前超界校验与预览标红（防撞轴）；未配置时服务端仅按 AxiDraw 档案告警放行
3. 填写设备名称，点击「保存配置」
...
# 按原尺寸 1:1 绘制（图形小于纸张绘图区域时不放大）
node cli.mjs plot input.svg --paper-size A4 --no-fit-page

# 自定义缩放比例绘制（如 130%，超出绘图区域的部分配合 --crop-to-margins 裁掉）
...
**最新版本 [v0.20.0](https://github.com/lymanzhang/Bit2AtomPlotWebUI/releases/tag/v0.20.0)**（2026-09-08 发布）：新增缩放模式三态（等比缩放到纸张 / 按原尺寸 1:1 / 自定义缩放比例 + 裁剪至边距）、SVG 真实尺寸自动检测（非 96dpi 导出也能按原尺寸绘制）、custom 硬件安全工作区配置（防撞轴校验）；修复「裁剪至边距」后图层过滤索引错位导致大量路径误删、导入 SVG 永久卡在「加载文件中」、自定义纸张回显为 USLetter 等问题。

历史版本 [v0.19.0](https://github.com/lymanzhang/Bit2AtomPlotWebUI/releases/tag/v0.19.0)（2026-09-08 发布）：新增按源文件归档的绘制任务日志（含图层信息、距离/速度/时长统计，便于项目管理）；新增排版设置（水平/垂直定位锚点 + 自定义偏移）；修复长时绘制中单条长动作被固定 150s 超时误杀（改为按估计时长动态计算）、LM 速率编码溢出导致运动引擎停摆（钳制 ≤24999 步/s）、深 FIFO 图形绘制结束时「电机未归位」误报（排空超时动态计算）、运动超时后命令队列错位导致笔不抬起、任务日志绘制距离与 UI 显示不一致（block 累加 + 步进密度换算）、标尺刻度数字拖拽中被选中变蓝等问题。
